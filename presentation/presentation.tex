...
\begin{itemize}
\item{$dp[row][col][time][last]$ --- максимальное суммарный вес частиц, который можно собрать}
...
\left\{
\begin{array}{l}
        (row+1, col_{row+1}, time+|col_{row}-col_{row+1}| + 1, 0)\\
 \mbox{если } col_{row} \neq col_{row+1}\\
  \\
        (row+1, col_{row+1}, time + 1, last + 1)\\
 \mbox{иначе }
\end{array}
\right.$}
\end{itemize}
...
\item{$row \cdot col^2 \cdot time \cdot last \approx 5 \cdot 10^8$}
\end{itemize}
\end{frame}
